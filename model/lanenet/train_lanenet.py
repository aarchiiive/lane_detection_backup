    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for inputs, binarys, instances in dataloaders[phase]:
                ...
                loss_t = loss[0].item() * inputs.size(0)
                count += 1
                
                if count % 32 == 0:
                    print("=================================")
                    print("iterations : {}, epochs : {}".format(count, epoch))
                    print("=================================")
                
                if count % 4 == 0 and phase == 'train':
                    # loss_b = accuracy_score(outputs["binary_seg_logits"], binarys)
                    # loss_i = accuracy_score(instances, outputs["instance_seg_logits"])
                    x_iter.append(count)
                    # y_loss.append(running_loss / count)
                    y_loss.append(loss_t)
                    # y_loss.append(loss)
                    # y_loss.append(loss_i)
                    # print("dataset_sizes[phase] :", dataset_sizes[phase])
                    print("Current Loss : {}".format(y_loss[-1]))
                ...
                if count % 100 == 0:
                    ...
                    # plt.ylabel("accuracy")
                    # plt.ylim(0, 1)
                    plt.savefig(os.getcwd() + "./iterations/losses_iter_{}.png".format(count))
